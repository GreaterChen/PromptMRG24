...
def parse_agrs():
    ...
    # Data input settings
    parser.add_argument('--image_dir', type=str, default='/mnt/chenlb/datasets/mimic_cxr/images', help='the path to the directory containing the data.')
    parser.add_argument('--ann_path', type=str, default='/mnt/chenlb/datasets/mimic_cxr/mimic_annotation_promptmrg_new.json', help='the path to the directory containing the data.')
    ...
    # Model settings 
    parser.add_argument('--load_pretrained', type=str, default="/home/chenlb/xray_compare_model/PromptMRG/results/promptmrg/A100/model_best.pth", help='pretrained path if any')
    ...
    parser.add_argument('--save_dir', type=str, default='results/promptmrg', help='the patch to save the models.')
...
def main():
    ...
    log, df = tester.test_blip()
    for key, value in log.items():
        print('\t{:15s}: {}'.format(str(key), value))
    
    df.to_csv(os.path.join(args.save_dir, 'test_results.csv'), index=False)
